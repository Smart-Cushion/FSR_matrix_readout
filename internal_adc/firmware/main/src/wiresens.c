...
size_t fsr_wiresens_packet_payload_size(uint16_t nodes_per_packet) {
    return WIRESENS_SEND_ID_SIZE + WIRESENS_START_IDX_SIZE +
           WIRESENS_READING_SIZE * nodes_per_packet +
           WIRESENS_PACKET_NUMBER_SIZE;
}

size_t fsr_wiresens_packet_serial_size(uint16_t nodes_per_packet) {
    return fsr_wiresens_packet_payload_size(nodes_per_packet) +
           WIRESENS_DELIMITER_SIZE;
}
...
esp_err_t fsr_wiresens_pack_packet(
    ...
    if (nodes_per_packet == 0 ||
        nodes_per_packet > FSR_WIRESENS_MAX_NODES_PER_PACKET) {
        return ESP_ERR_INVALID_ARG;
    }
...
void fsr_wiresens_init(fsr_wiresens_cfg_t cfg) {
    ...
    ESP_ERROR_CHECK(
        cfg.nodes_per_packet <= FSR_WIRESENS_MAX_NODES_PER_PACKET
            ? ESP_OK
            : ESP_ERR_INVALID_ARG
    );
    ESP_ERROR_CHECK(
        WIRESENS_FRAME_NODE_CNT % cfg.nodes_per_packet == 0
            ? ESP_OK
            : ESP_ERR_INVALID_ARG
    );
    ...
    ESP_ERROR_CHECK(uart_driver_install(
        cfg.uart_num, 256, cfg.tx_buffer_size, 0, nullptr, 0
    ));
...
esp_err_t fsr_wiresens_send_frame(const uint16_t *frame) {
    ...
    size_t packet_size = fsr_wiresens_packet_serial_size(nodes_per_packet);
    for (uint16_t start_idx = 0; start_idx < WIRESENS_FRAME_NODE_CNT;
         start_idx += nodes_per_packet) {
        ESP_RETURN_ON_ERROR(
            fsr_wiresens_pack_packet(
                tx_packet_buf, sizeof(tx_packet_buf), wiresens_cfg.sensor_id,
                start_idx, &frame[start_idx], nodes_per_packet, packet_number
            ),
